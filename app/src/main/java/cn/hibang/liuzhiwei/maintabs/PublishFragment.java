...
public class PublishFragment extends BaseFragment implements
	...
	public PublishFragment(BaseApplication application, Activity activity,
			Context context) {
	...
	private void initData() {
		...
		for (int i = 0; i < groupList.size(); i++) {
			...
			if (i == 0) {
				...
				List<MyPubHelpMe> helpMeList = DBManage.getAllCPubHelpMe();
				for (int n = helpMeList.size(); n > 0; n--) {
					childTemp
							.add(new TestChild(helpMeList.get(n - 1)
									.getReqItemName()
									+ "("
									+ helpMeList.get(n - 1).getReqDetail()
									+ ")", 0, ""));
				}
			} else if (i == 1) {
				childTemp = new ArrayList<TestChild>();
